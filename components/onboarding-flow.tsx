...
export default function OnboardingFlow() {
  ...
  // Only keep the current step's history for backend context
  function getHistoryUpToCurrent() {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/webhook/onboarding", {
        ...
        body: JSON.stringify({ sessionId, history: newSteps }),
      });
      const data = await res.json();
      setSteps([...newSteps, { role: "agent" as const, message: data.message }]);
      setCurrentStep(newSteps.length + 1); // move to the new agent message
  ...
  // Only show the current step (agent or user)
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-50 px-2 py-6">
      ...
      <div className="flex flex-col justify-between items-center max-w-md sm:max-w-lg w-full min-h-[500px] bg-white rounded-2xl shadow-lg border border-gray-200 p-0 relative">
        ...
        <div className="flex-1 flex flex-col justify-center items-center w-full px-4 py-12">
          {currentMessage && currentMessage.role === "agent" && (
            <div className="text-center text-gray-800 text-lg font-medium mb-6 whitespace-pre-line">
              {currentMessage.message}
            </div>
          )}
          {!done && currentMessage && currentMessage.role === "agent" && (
            <form
              className="w-full max-w-xl flex flex-col sm:flex-row gap-3 items-center justify-center"
              onSubmit={handleSend}
            >
              <input
                type="text"
                className="max-w-xl w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-yellow-200 text-center"
